    ...
    for i in range(100):
        ...
        for state in simgr.stashes['active']:
            ...
            #     print(state.satisfiable(extra_constraints=[count > 3]))
                # print(i,count)
            if state.regs.rip.args[0]==0x4006c6:
                print('get')
...
p = angr.Project("/tmp/fuzz/example/jhead-master/jhead")
s = p.factory.blank_state(addr=0x403DB0)
s.regs.rdi = 0x612350
s.memory.store(0x612350, 'jhead\x00')
simgr = p.factory.simulation_manager(s)
res = simgr.explore(find=0x403E45 )
print(res)
